...
class Database:
    ...
    async def add_filter(self, grp_id, text, reply_text, btn, file, alert):
        ...
        try:
            ...
            print('Kaydedilemedi, veri tabanınızı kontrol edin')
    ...
    async def delete_filter(self, message, text, group_id):
        ...
        if query == 1:
            ...
            await message.reply_text(f"'`{text}`' silindi. Artık o filtreye yanıt vermeyeceğim.", quote=True, parse_mode=pyrogram.enums.ParseMode.MARKDOWN)
        else:
            await message.reply_text("Bu filtre bulunamadı!", quote=True)

    async def del_all(self, message, group_id, title):
        if str(group_id) not in self.client1.list_collection_names():
            await message.edit_text(f"Kaldırılacak hiçbir şey yok {title}!")
            return
        
        mycol = self.client1[str(group_id)]
        try:
            mycol.drop()
            await message.edit_text(f"{title} kaynağındaki tüm filtreler kaldırıldı")
        except:
            await message.edit_text(f"Gruptan tüm filtreler kaldırılamadı!")
            return

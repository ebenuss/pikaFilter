...
async def addfilter(client, message):
    ...
    if not userid:
        return await message.reply(f"Siz anonim bir yöneticisiniz. PM'de /connect {message.chat.id} adresini kullanın")
    ...
    if len(args) < 2:
        await message.reply_text("Komut Tamamlanmadı :(", quote=True)
        return
    ...
    if not message.reply_to_message and len(extracted) < 2:
        await message.reply_text("Filtrenizi kaydetmek için biraz içerik ekleyin!", quote=True)
        return
    ...
    if (len(extracted) >= 2) and not message.reply_to_message:
        ...
        fileid = None
        if not reply_text:
            await message.reply_text("Tek başına düğmelere sahip olamazsın, onunla birlikte gidecek bir metin ver!", quote=True)
            return
    ...
    await message.reply_text(f"**{title}**'e '{text}' filtresi eklendi", quote=True, parse_mode=enums.ParseMode.MARKDOWN)
